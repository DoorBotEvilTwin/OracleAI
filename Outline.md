...
*   **F0410: Akashic Record - Peak Functionality & Intelligence:**
    *   [F0411] **Full Vector Store ("The Sanctum"):**
        *   Integration of a local Vector DB (e.g., ChromaDB, FAISS with SQLite metadata, LanceDB).
        *   Automated embedding generation (Sentence Transformers via `EMBEDDING_MODEL_NAME_OR_PATH`) for Chronicle entries, Citadel content (Summaries, Entities, Facts), and imported Library documents.
        *   Semantic search fully integrated into Context Scaler.
    *   [F0412] **Knowledge Graph ("The Citadel" Refined):**
        *   More advanced relationship extraction capabilities (by Shaman/NLU).
        *   Graph traversal queries integrated into Context Scaler.
        *   (v0.4+): Potential migration to dedicated Graph DB (Neo4j, etc.) if SQLite simulation proves insufficient for complex graph algorithms.
    *   [F0413] **Memory Bank Context Scaler (RAG Engine) - Advanced Techniques:**
        *   Full hybrid search (keyword + semantic + graph).
        *   Advanced RAG techniques (inspired by Langroid, RAG Notes from Big Dataset): Query transformation (HyDE, step-back), fusion ranking (Reciprocal Rank Fusion - RRF), cross-encoder reranking (if performant local models available), flexible context windowing (Sentence Window, Parent Document/Auto-Merging).
    ...
    *   [F0415] **RAPTOR-like Hierarchical Indexing:** For very large imported documents/datasets, Shaman builds a tree of summaries at different abstraction levels, stored in The Citadel. Context Scaler can traverse this tree.
    *   [F0416] **Chrono-Contextual Echo Layer (CCEL - v0.4+):**
...
*   **F0420: Agent Evolution & Advanced Modes:**
    ...
    *   [F0423] **Neural Graffiti (Experimental for Shaman, potentially other agents):**
        *   ENV toggler for modes: `none`, `pytorch_direct` (e.g., Gemma 1B), `pre_gguf_bias`, `post_gguf_filter`.
        *   Persistence of SprayLayer state and memory_bank (via `profile.db` or dedicated files).
        *   `GRAFFITI_AGENT_ENABLED`, `GRAFFITI_AGENT_PYTORCH_MODEL_ID`, etc. in `.env`.
        *   (v0.4+ Research): Feasibility of Llama.cpp modification for GGUF graffiti.
    *   [F0424] **Prompt Graffiti (Shaman-Suggested Prompt Evolution):** Shaman analyzes agent performance/drift and suggests system prompt component changes to the user for approval.
    *   [F0425] **Black Magick Mindmap Modifier (BMMM - Experimental Novelty Layer):**
        *   ENV toggler for modes: `none`, `Chaotic Brainstorming`, `Divination Arts`, `Sigil Seed` (with optional image generation to Akashic), `Matrix Hacking`.
        *   `BMMM_DEPTH_SCALE` (0.0-1.0) in `.env`.
        *   Interoperable with Neural Graffiti and Prompt Graffiti.
...
*   **F0430: Enhanced Modalities & Integrations:**
    ...
    *   [F0432] **ComfyUI Integration (Text-to-Image - v0.4+):** Agents can generate images based on prompts/descriptions. Displayed in GUI. Configurable timing (real-time vs. pre-loading).
    *   [F0433] **"Library" Feature (LlamaIndex Powered):**
        *   User-managed `OracleAI\Library\` folder (path configurable in `.env`).
        *   `scripts/library_manager.py`: Uses LlamaIndex `DocumentLoaders` (including PDF/EPUB via Langchain loaders if integrated) to ingest, chunk, and create a dedicated vector index (part of The Sanctum) for Library content.
        *   Shaman "Local Library Search" mode and "Deep Web + Library Search" mode using this LlamaIndex query engine as a tool.
    *   [F0434] **LiteLLM Integration (v0.4+):** Refactor `llm_interface.py` for flexible backend support (Ollama, LM Studio, cloud APIs if user opts-in).
*   **F0440: Polished GUI & User Experience:**
    *   [F0441] Full "Oracle Temple" / "Techno-Mysticism" aesthetic realized.
    *   [F0442] Comprehensive Akashic Record Management Interface (browse, search, edit, visualize relationships, manage MemCon cache).
    *   [F0443] Advanced dashboards for Shaman tasks, agent status, emotional trends, NLU insights.
    *   [F0444] Star Wars-like holographic planet icons for Shaman's Hyperspace Journey.
    *   [F0445] Contextual Memory Injection Preview toggler (qvink-inspired).
*   **F0450: System Robustness & Documentation:**
    *   [F0451] Comprehensive end-user and developer documentation (v0.4 ReadMe, v0.4 Design Document including fully updated Developer Context Appendix H).
    *   [F0452] Automated setup script (`setup_oracleai.bat/.sh`) for easier installation of dependencies (Python, PyTorch, NLTK data, etc.) and initial configuration file generation.
    *   [F0453] Rigorous testing, performance profiling, and optimization across all features.
*   **F0460: Advanced Concepts (v0.4+ Research & Stretch Goals):**
